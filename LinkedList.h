...
public:
    ...
    LLNode* previous;
    LLNode* next;
    LLNode() : data(nullptr), previous(nullptr), next(nullptr) {}
...
public:
    ...
    */
    ...
    LLNode<ptr_type>* find_node(ptr_type* data1);
...
template<class ptr_type>
LLNode<ptr_type> *LinkedList<ptr_type>::find_node(ptr_type *data1) {
    LLNode<ptr_type> *temp = this->head;
    while (data1 != temp->data || temp->data != nullptr) {
        temp = temp->next;
    }
...
template<class ptr_type>
bool LinkedList<ptr_type>::is_end_of_list() {
